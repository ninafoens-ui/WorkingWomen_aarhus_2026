...
data_1860_relevant <- data_1860 %>% 
  filter(!stilling %in% c(
    "Familiemedlem",
    "Fattiglem",
    "Forsørges af familiemedlem",
    "NA",
    "Husholderske",
    "Huskone",
    "Enke",
...
write_csv(
  data_1860_relevant,
  "data_output/data_1860_relevant.csv"
)

coloumn_occupation_relevant_60 <- data_1860_relevant %>% 
  count(stilling, sort = TRUE) %>% 
  slice_max(n, n = 10) %>% 
  ggplot(aes(x = reorder(stilling, n), y = n)) +
...
print(coloumn_occupation_relevant_60)

ggsave("fig_output/coloumn_occupation_relevant_60.tiff",
       plot = coloumn_occupation_relevant_60,
       width = 10,
       height = 5
)
